...
//Code Here
let totalPopulation = populations.reduce((accumulator, element, index, array) => {
...
let myStrongest = monstersInYourPocket.filter((element, index, array) => {
  return (element["CP"] > 200 ? true : false)
});
...
let bobsTotal = purchases.filter((element, index, array) => {
  return (element["owner"] === "Bob" ? true: false)
})
.reduce((accumulator, element, array, index) => {
